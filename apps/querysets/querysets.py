...
class CommentItemQueryset(MySqlQueryset, AbstractQueryset):
    ...
    def __init__(self, settings, table_name, db_conn = None, **kw):
        """We may want to resuse a db_conn, not sure yet
           We need to pass the tablename since it may be different
           based on the API key
        """
        super(CommentItemQueryset, self).__init__(settings, db_conn)
        self.table_name = table_name
        self.image_queryset = ImageQueryset(settings, self.table_name + '_images', db_conn);
...
class CommentQueryset(MySqlApiQueryset):
    ...
    def __init__(self, settings, table_name, db_conn = None, **kw):
        ...
        """
        super(CommentQueryset, self).__init__(settings, db_conn = db_conn, **kw)
        self.table_name = table_name
...
class UserQueryset(MySqlApiQueryset):
    ...
    def __init__(self, settings, db_conn = None, **kw):
        ...
        """call our MySql __init__.
           This will create a connection for us if we need it
        """
        super(UserQueryset, self).__init__(settings, db_conn = db_conn, **kw)
...
class ImageQueryset(MySqlApiQueryset):
    ...
    def __init__(self, settings, table_name, db_conn = None, **kw):
        ...
        """call our MySql __init__.
           This will create a connection for us if we need it
        """
        super(ImageQueryset, self).__init__(settings, db_conn=db_conn, **kw)
        self.table_name = table_name
