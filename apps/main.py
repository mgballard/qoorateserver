...
config = {
    ...
    'handler_tuples': [ ## Set up our routes
        (r'^/post_embed', FeedHandler),
        (r'^/embed.php', EmbedHandler),
        (r'^/uploader', TemporaryImageUploadHandler),
        (r'^/php/tmp/(?P<file_name>.+)_s.jpg$', TemporaryImageViewHandler),
        (r'^/oauth/(?P<provider>.+)/(?P<action>.+)$', QoorateOAuthHandler),
    ],
